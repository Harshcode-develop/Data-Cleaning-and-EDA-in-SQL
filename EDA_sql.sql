...
-- Max percentage laid off which is 100 percent
SELECT * FROM layoffs_staging2 WHERE percentage_laid_off=1 ORDER BY funds_raised_millions DESC;

SELECT company, SUM(total_laid_off) AS total_laidoff FROM layoffs_staging2 GROUP BY company ORDER BY 2 DESC;
...
SELECT company, SUM(total_laid_off) FROM world_layoffs.layoffs_staging2 GROUP BY company ORDER BY 2 DESC LIMIT 10;
...
SELECT YEAR(date), SUM(total_laid_off) FROM world_layoffs.layoffs_staging2 GROUP BY YEAR(date) ORDER BY 1 ASC;
...
-- use it in a CTE so to query off of it
WITH DATE_CTE AS 
